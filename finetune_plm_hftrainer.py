...
def main(config):
    ...
    # Ensure device count is not zero
    device_count = max(1, torch.cuda.device_count())
    total_batch_size = config['batch_size_per_device'] * device_count
    ...
    training_args = TrainingArguments(
        ...
        evaluation_strategy='epoch',  # Wandb로 로깅하도록 설정
        save_strategy='epoch',        # Save strategy를 'epoch'으로 변경
        logging_steps=n_total_iterations // 100,
        save_steps=n_total_iterations // config['n_epochs'],
        load_best_model_at_end=True,
        report_to='wandb',  # Wandb로 로깅하도록 설정
    )
    ...
    # Pass the wandb object to the Trainer
    trainer = Trainer(
        model=model,
        args=training_args,
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Train PLM model with configuration file")
